...
class TestCart:
    ...
    def test_add_to_cart(self):
        ...
        assert cart.check_cart(), \
            "Product was not added to shopping cart"

    def test_add_multiple_products_to_cart(self):
        cart = CartAction(self.driver)

        cart.add_products_from_excel(
        ...
        assert cart.verify_products_displayed(), \
            "Products are not displayed in shopping cart"
    ...
    def test_cart_total_amount(self):
        ...
        assert cart.verify_subtotal_displayed(), \
            "Cart subtotal and total amount is not displayed"
    ...
    def test_empty_cart_message(self):
        ...
        assert ReadConfig.get_empty_cart_msg() in \
    ...
    def test_invalid_coupon_code(self, coupon):
        ...
        assert ReadConfig.get_invalid_coupon_msg() in \
               cart.get_validation_message(), \
               "Invalid coupon message is not displayed"

    @pytest.mark.parametrize(
        "giftcard",
        ["gift1234", "giftcard123"]
    )
    ...
    def test_update_cart_quantity(self):
        ...
        cart.update_quantity(
            ReadConfig.get_updated_quantity()
        )

        cart.click_update_shopping_cart()

        assert cart.verify_updated_quantity(
            ReadConfig.get_updated_quantity()
        ), \
    ...
    def test_remove_product_from_cart(self):
        ...
        assert cart.verify_removed_product(), \
